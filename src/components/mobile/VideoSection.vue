<template>
	<section class="compilation">
		<div class="header">
            <router-link class="header__button-back" to="/">
		...
		<div class="compilation__about">
			<h2 class="compilation__title">{{blocks.title}}</h2>
			<p class="compilation__text">{{blocks.text}}</p>
		</div>
...
<script>
// import Header from "../components/HeaderCompilation.vue";
...
export default {
	...
	data() {
		return {
			cards: data,
			pages: ['mostFriends', 'mostProfitable', 'popularPaid', 'mostFriendsDay', 'mostFriendsWeek']
		};
	},
	computed: {
		block() {
			return blocks[this.pages.findIndex(el => el === path)]
		}
	},
    methods: {
